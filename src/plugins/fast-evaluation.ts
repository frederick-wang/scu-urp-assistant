// 一键评教插件
interface CheckboxWrapperSelectors {
  [key: string]: string
}

interface QuestionsNumberRange {
  [key: string]: number[]
}

const evaluationInterval = 1000 * 61
const checkboxWrapperSelectors: CheckboxWrapperSelectors = {
...
const questionsNumberRange: QuestionsNumberRange = {
    ...
    success: () => {
      list.forEach(
        (
          ...
          if (checkboxWrapperSelectors[type]) {
            const selector = checkboxWrapperSelectors[type]
      ...
      for (const key in checkboxWrapperSelectors) {
        const selector = checkboxWrapperSelectors[key]
...
function evaluate(index: number) {
  ...
  window.$.ajax({
    ...
    success: data => {
      ...
      if (questionsNumberRange[questionnaireName]) {
        const range = questionsNumberRange[questionnaireName]
